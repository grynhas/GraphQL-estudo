...
class Pet {
  lista() {
    const sql = `SELECT Pets.id, Pets.nome, Pets.tipo, Pets.observacoes, Clientes.id as donoId, 
  ...
  buscaPorId(res, id) {
    ...
    FROM Pets INNER JOIN Clientes Pets.WHERE id=${parseInt(id)} AND Clientes.id = Pets.donoId`
